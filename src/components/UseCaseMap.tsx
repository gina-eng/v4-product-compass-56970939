...
const UseCaseMap = ({ title, data, onChange, readOnly = true }: UseCaseMapProps) => {
  ...
  const getFieldValue = (field: keyof UseCaseMapData) => {
    const source = (data || {}) as Record<string, unknown>;
    const aliases: Record<keyof UseCaseMapData, string[]> = {
      problema: ["problema", "p_problema", "pProblema"],
      persona: ["persona", "p_persona", "pPersona"],
      alternativa: ["alternativa", "a_alternativa", "aAlternativa"],
      why: ["why", "w_why", "wWhy"],
      frequencia: ["frequencia", "frequência", "f_frequencia", "fFrequencia"],
    };
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className={readOnly ? "space-y-3" : "grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4"}>
          {fields.map((field) => (
            <div
              key={field.key}
              className={`p-4 rounded-lg border ${field.color} ${readOnly ? "flex flex-col md:flex-row md:items-start md:gap-4" : "h-full"}`}
            >
              <div className={`text-base font-bold text-foreground block ${readOnly ? "md:min-w-[180px] md:mb-0" : "mb-3 border-b pb-2"}`}>
                {field.label}
              </div>
              {readOnly ? (
                <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed flex-1">
                  {getFieldValue(field.key as keyof UseCaseMapData) || "Não definido"}
                </div>
              ) : (
                <textarea
